...
class ETM(nn.Module):
    ...
    def encode(self, bows):
        ...
        return mu_theta, logsigma_theta, kl_theta, q_theta
    ...
    def get_theta(self, normalized_bows):
        mu_theta, logsigma_theta, kld_theta, q_theta = self.encode(normalized_bows)
        z = self.reparameterize(mu_theta, logsigma_theta)
        theta = F.softmax(z, dim=-1) 
        return theta, kld_theta, q_theta
    ...
    def forward(self, bows, normalized_bows, theta=None, aggregate=True):
        ## get \theta
        if theta is None:
            theta, kld_theta, _ = self.get_theta(normalized_bows)
        else:
            kld_theta = None
