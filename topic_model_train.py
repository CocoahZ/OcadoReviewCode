...
parser.add_argument('--dataset', type=str, default='min_df_100', help='name of corpus')
parser.add_argument('--data_path', type=str, default='data/min_df_100', help='directory containing data')
parser.add_argument('--emb_path', type=str, default='data/min_df_100/embeddings.txt', help='directory containing word embeddings')
...
parser.add_argument('--num_words', type=int, default=15, help='number of words for topic viz')
...
def visualize(m, show_emb=True):
    ...
    queries = ['ocado', 'bag', 'price', 'service', 'delivery']
...
def evaluate(m, source, tc=False, td=False):
    ...
    with torch.no_grad():
        ...
        for idx, ind in enumerate(indices_1):
            ...
            theta, _, _ = m.get_theta(normalized_data_batch_1)
    ...
    with torch.no_grad():
        ...
        for idx, ind in enumerate(indices):
            ...
            if args.bow_norm:
                ...
                normalized_data_batch = data_batch
            theta, _, embedding_text = model.get_theta(normalized_data_batch)
            # print(torch.argmax(theta, dim=1))
            total_theta.append(theta.detach().cpu().numpy())
            total_embedding_text.append(embedding_text.detach().cpu().numpy())
        ...
        total_embedding_text = np.array(total_embedding_text)
        total_theta = np.array(total_theta)
        np.save('./results/embedding_text_{}.npy'.format(args.num_topics), total_embedding_text)
        np.save('./results/theta_{}.npy'.format(args.num_topics), total_theta)
